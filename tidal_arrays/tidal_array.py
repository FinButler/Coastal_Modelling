...
outputdir = 'outputs'
if not os.path.exists('idealised_channel.msh'):
    os.system('gmsh -2 tidal_0.geo -o idealised_channel.msh')
mesh2d = Mesh('idealised_channel.msh')
site_ID = 2
site_ID_2 = 3
# mesh PhysID for subdomain where turbines are to be sited
...
h_viscosity = Function(P1_2d).interpolate(conditional(le(x[0], 200), 201-x[0], 1.0))
...
farm_options.turbine_coordinates = [[Constant(x), Constant(y)]
                                   for x in numpy.arange(1020, 1480, 120)
                                   for y in numpy.arange(620, 880, 80)]

farm_options_2 = copy.deepcopy(farm_options)
farm_options_2.turbine_coordinates = [[Constant(x), Constant(y)]
                                   for x in numpy.arange(1020, 1480, 120)
                                   for y in numpy.arange(120, 380, 80)]


V = get_functionspace(mesh2d, 'CG', 1)
quadratic_drag = Function(V, name='quadratic_drag').assign(0.0)

coordinates = mesh2d.coordinates.dat.data[:]
x_, y_ = coordinates[:, 0], coordinates[:, 1]

quadratic_drag_values = [Constant(x, domain=mesh2d) for x in [0.0025, 0.001, 0.005, 0.0025]]

mask_values = [(x_ <= 900).astype(float),
               np.logical_and(np.logical_and(x_ > 900, x_ < 1500), y_ < 500).astype(float),
               np.logical_and(np.logical_and(x_ > 900, x_ < 1500), y_ >= 500).astype(float),
               (x_ >= 1500).astype(float)]
...
update_n(quadratic_drag, quadratic_drag_values)
...
options.discrete_tidal_turbine_farms[site_ID_2] = [farm_options_2]
...
solver_obj.add_new_field(quadratic_drag, 'quadratic_drag_2d',
                             quadratic_drag.name(),
                             'quadratic_drag', unit='')
solver_obj.create_equations()
VTKFile(outputdir + '/friction/quadratic_drag.pvd').write(solver_obj.fields.quadratic_drag_2d)
...
cb_farm = turbines.TurbineFunctionalCallback(solver_obj, append_to_log=False)
